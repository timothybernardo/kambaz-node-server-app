/**
 * Data Access Object for quizzes and attempts.
 * Exposes all Mongoose queries: quiz CRUD (with UUID generation and
 * auto-calculated total points on update), publish toggle, and attempt
 * creation/lookup/counting. Isolates database logic so routes never
...
export default function QuizzesDao() {
  ...
  const updateQuiz = async (quizId, updates) => {
    ...
    return QuizModel.findByIdAndUpdate(quizId, { $set: updates }, { returnDocument: 'after' });
  };
  ...
  const publishQuiz = (quizId, published) =>
    QuizModel.findByIdAndUpdate(quizId, { $set: { published } }, { returnDocument: 'after' });
